...
simulate_burnin_single <- function(partition,
  ...
  #   lo <- loess(y ~ x, data.frame(x = seq_len(nrow(chain$draws)), y = allmeans[, eff]))#1:nrow(chain$draws)
  ...
  chain <- draw_Metropolis_single(theta, partition, nodes, effects, objects,
                                  1, 1, num.steps, neighborhood,
                                  numgroups.allowed, numgroups.simulated,
                                  sizes.allowed, sizes.simulated,
                                  return.all.partitions = FALSE)

  # Ensure chain$draws is a non-empty matrix or data.frame
  if (is.null(nrow(chain$draws)) || nrow(chain$draws) == 0) {
    stop("ERROR: chain$draws must be a non-empty matrix or data.frame")
  }

  n_draws <- nrow(chain$draws)
  allmeans <- matrix(0, nrow = n_draws, ncol = num.effects)
  ...
  # Smooth the curves (useful for small samples)
  smoothedmeans <- allmeans
  for (eff in seq_len(num.effects)) {
    df <- data.frame(x = seq_len(n_draws), y = allmeans[, eff])
