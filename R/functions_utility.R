...
calculate_confidence_interval <- function(vector, conf) {
  ...
  sd <- sd(vector)
  n <- length(vector)
  error <- qt((conf + 1) * 0.5, df = length(vector) - 1) * sd / sqrt(length(vector))
...
#     first.partition <- 1 + rbinom(num.nodes, as.integer(num.nodes * 0.5), 0.5)
#   } else {
...
#     if (is.null(sizes.allowed)) {
#       first.partition <- 1 + rbinom(num.nodes.o, as.integer(num.nodes * 0.5), 0.5)
#     } else {
#       smin <- min(sizes.allowed)
#       cpt <- 0
#       g <- 1
