# ==============================================================================
# File    : R/InitErgmProposal.ErpmMergeStep.R
# Purpose : Register the ERPM MCMC proposal 'ErpmMergeStep' for partition moves
#           under the ~ b1part constraint (bipartite actor-group network).
# Project : ERPM / ERGM extensions
# ==============================================================================

#' ERGM proposal: ErpmMergeStep
#'
...
#' }
#'
#' The move selects two distinct groups g1 and g2 and merges g2 into g1 by moving
#' every actor currently in g2 to g1. In the bipartite encoding, this corresponds
#' to a multi-toggle move of size 2*|g2|:
#' for each actor i in g2, remove (i, g2) and add (i, g1).
#'
#' @details
...
#' }
#'
#' @keywords ERGM MCMC proposal ERPM b1part merge
#' @export
InitErgmProposal.ErpmMergeStep <- function(nw, ...){
  list(
    name    = "ErpmMergeStep",  # Must match MH_ErpmMergeStep in C.
    pkgname = "ERPM",
    inputs  = NULL,
    iinputs = NULL
  )
