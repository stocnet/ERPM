  ...
  for (e in 1:num.effects) {
    ...
    if (effect.name == "tie") {
      if (length(groups) > 0) {
        ...
        statistics[e] <- sum(0.5 * adjacency * net)
      }else {
        statistics[e] <- 0
      }
    ...
    if (effect.name == "tie_X_diff") {
      if (length(groups) > 0) {
        ...
        d <- as.matrix(d)
        statistics[e] <- sum(0.5 * adjacency * net * d)
      }else {
        statistics[e] <- 0
      }
    ...
    if (effect.name == "bipartite_tie") {
      if (length(groups) > 0) {
        ...
        for (g in 1:dim(binet)[2]) {
          ...
          stat <- stat + sum(0.5 * adjacency[members, members])
        }
        statistics[e] <- stat
      }else {
        statistics[e] <- 0
    ...
    if (effect.name == "same") {
      if (length(groups) > 0) {
        ...
        d <- (d == 0)
        diag(d) <- 0
        statistics[e] <- sum(0.5 * adjacency * d)
      } else {
        statistics[e] <- 0
      }
    ...
    if (effect.name == "same_norm") {
      if (length(groups) > 0) {
        ...
        d <- (d == 0)
        diag(d) <- 0
        statistics[e] <- sum(0.5 * adjacency_norm * d)
      } else {
        statistics[e] <- 0
      }
    ...
    if (effect.name == "diff_norm") {
      if (length(groups) > 0) {
        ...
        d <- dist(nodes[, object.name])
        d <- as.matrix(d)
        stat <- stat + sum(0.5 * adjacency_norm * d)
      }else {
        statistics[e] <- 0
      }
    }
    ...
    if (effect.name == "number_attributes") {
      if (length(groups) > 0) {
        ...
        d <- unlist(lapply(1:num.groups,
                           function(x) {
                             return(length(unique(nodes[which(partition == x), object.name])))
                           }))
        statistics[e] <- sum(d)
      } else {
        statistics[e] <- 0
    ...
    if (effect.name == "tie") {
      ...
      for (o in 1:num.obs) {
        if (length(groups[[o]]) > 0) {
          ...
          statistics[e, o] <- sum(0.5 * adjacencies[[o]] * net2)
        }
      }
    }
    ...
    if (effect.name == "tie_X_diff") {
      ...
      for (o in 1:num.obs) {
        if (length(groups[[o]]) > 0) {
          ...
          net2 <- net[as.logical(presence.tables[, o]), as.logical(presence.tables[, o])]
          statistics[e, o] <- sum(0.5 * adjacencies[[o]] * net2 * d)
        }
      }
    }
      ...
      for (o in 1:num.obs) {
        if (length(groups[[o]]) > 0) {
          ...
          for (g in 1:dim(binet2)[2]) {
            ...
            statistics[e, o] <- statistics[e, o] + sum(0.5 * adjacencies[[o]][members, members])
          }
        }
      }
    }
    ...
    if (effect.name == "same") {
      ...
      for (o in 1:num.obs) {
        if (length(groups[[o]]) > 0) {
          ...
          d <- d == 0
          diag(d) <- 0
          statistics[e, o] <- sum(0.5 * adjacencies[[o]] * d)
        }
      }
    }
    ...
    if (effect.name == "same_norm") {
      ...
      for (o in 1:num.obs) {
        if (length(groups[[o]]) > 0) {
          ...
          d <- (d == 0)
          diag(d) <- 0
          statistics[e, o] <- sum(0.5 * adjacencies_norm[[o]] * d)
        }
      }
    }
      ...
      for (o in 1:num.obs) {
        if (length(groups[[o]]) > 0) {
          d <- unlist(lapply(1:nums.groups[o],
                             function(x) {
                               return(length(unique(nodes[which(partitions[, o] == x), att])))
                             }))
    ...
    if (effect.name == "inertia_total") {
      ...
      for (o in 2:num.obs) {
        if (length(groups[[o]]) > 0) {
          ...
          statistics[e, o] <- sum(0.5 * adj_total)
        }
      }
    }
    ...
    if (effect.name == "inertia_total_X_diff") {
      ...
      for (o in 2:num.obs) {
        if (length(groups[[o]]) > 0) {
          ...
          adj2[as.logical(presence.tables[, o]),  as.logical(presence.tables[, o])]   <- adjacencies[[o]]
          adj_total <- adj_total + adj2
          statistics[e, o] <- sum(0.5 * adj_total * d)
        }
      }
    }
