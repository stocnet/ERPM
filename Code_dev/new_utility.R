...
compute_number_same_pairs <- function( partition, attribute) {
  ...
  indsameties <- sum( rowSums(distances * partition) > 0 )
  
  res <- list(number_same_ties = sameties,
              averagepergroup_number_same_ties = avgroupsameties,
              individual_number_same_ties = indsameties)
...
# Mettre le type 
compute_number_ties_dyadic <- function(partition_obs, net_attribute) {
...
compute_number_similar_pairs <- function(partition_obs, attribute, threshold, stat= 'average') {
  ...
  res <- list(number_same_ties = sameties_obs,
              averagepergroup_number_same_ties = avgroupsameties_obs,
              individual_number_same_ties = indsameties_obs)
...
CUG <- function(observation, fun, partition_sample ) {
  ...
  stat_sample <- matrix(0,length(partition_sample),length(stat_obs))
  
  for (stat in 1:length(stat_obs)){
    for(s in 1:length(partition_sample)) {
      stat_sample[stat,s] <- fun(partition_sample[[s]]$partition)
    }
  }

  for (stat in 1:length(stat_obs)){
# Comment garder le nom des stat
  } 
