...
plot_partition <- function(partition, title = NULL, group.color = NULL,
  ...
  possible.color <- c("pink" ,"cyan" ,"yellow",'green',"black", "magenta","gray" )
  possible.shape <-  c( "circle","square","rectangle","raster","crectangle" , "sphere","csquare","vrectangle")
    ...
    # set colors for groups
        if (is.null(group.color)) {
          g.colors <- brewer.pal(max(partition), name = "Dark2")
        }else {
