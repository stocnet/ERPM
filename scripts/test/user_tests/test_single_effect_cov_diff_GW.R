# ======================================================================================
# Fichier : scripts/test/user_tests/tests_single_effect_cov_diff_GW.R
#
# Objet   : Test single effect cov_diff_GW with different options
#
# ======================================================================================
...
dry <- erpm(partition_mix ~ cov_diff_GW("bin_att"), 
            nodes = nodes_df,
            eval_call = FALSE, verbose = TRUE)
print(summary(dry[[2]], constraints = ~ b1part)) # should be 0
dry <- erpm(partition_balanced ~ cov_diff_GW("bin_att"), 
            nodes = nodes_df,
            eval_call = FALSE, verbose = TRUE)
print(summary(dry[[2]], constraints = ~ b1part)) # should be -0.5
dry <- erpm(partition_full ~ cov_diff_GW("bin_att"), 
            nodes = nodes_df,
            eval_call = FALSE, verbose = TRUE)
print(summary(dry[[2]], constraints = ~ b1part)) # should be -1.53125
dry <- erpm(partition_singleton ~ cov_diff_GW("bin_att"),
            nodes = nodes_df,
            eval_call = FALSE, verbose = TRUE)
print(summary(dry[[2]], constraints = ~ b1part)) # should be 0 

# baseline test - category attribute
#dry <- erpm(partition_mix ~ cov_diff_GW("bin_cat"),
#            nodes = nodes_df,
#            eval_call = FALSE, verbose = TRUE) # should be an error

# option test - lambda = 3 
dry <- erpm(partition_mix ~ cov_diff_GW("bin_att", lambda = 3), 
            nodes = nodes_df,
            eval_call = FALSE, verbose = TRUE)
print(summary(dry[[2]], constraints = ~ b1part)) # should be 0
dry <- erpm(partition_balanced ~ cov_diff_GW("bin_att", lambda = 3), 
            nodes = nodes_df,
            eval_call = FALSE, verbose = TRUE)
print(summary(dry[[2]], constraints = ~ b1part)) # should be -0.3333
dry <- erpm(partition_full ~ cov_diff_GW("bin_att", lambda = 3), 
            nodes = nodes_df,
            eval_call = FALSE, verbose = TRUE)
print(summary(dry[[2]], constraints = ~ b1part)) # should be -1.485597
dry <- erpm(partition_singleton ~ cov_diff_GW("bin_att", lambda = 3),
            nodes = nodes_df,
            eval_call = FALSE, verbose = TRUE)
print(summary(dry[[2]], constraints = ~ b1part)) # should be 0 
...
fit_ergm <- ergm( nw ~ cov_diff_GW("bin_att"),
                  constraints = ~b1part, 
                  estimate="MLE", 
                  control=ctrl_A)
print(summary(fit_ergm))

fit_erpm <- erpm(partition_balanced ~ cov_diff_GW("bin_att"),
                 nodes = nodes_df,
                 estimate="MLE", 
                 control=ctrl_A) 
...
# option case TODO
nw <- make_nw_from_partition(partition_balanced,nodes_df)
fit_ergm <- ergm( nw ~ cov_diff_GW("bin_att", lambda=3),
                  constraints = ~b1part, 
                  estimate="MLE", 
                  control=ctrl_A)
print(summary(fit_ergm))

fit_erpm <- erpm(partition_balanced ~ cov_diff_GW("bin_att", lambda=3),
                 nodes = nodes_df,
                 estimate="MLE", 
                 control=ctrl_A) 
print(summary(fit_erpm))
