...
#                         contre une référence analytique (énumération explicite).
#   - PHASE 2 (EQUIV)   : vérifier summary(nw) == summary(ERPM-traduit) (si erpm() dispo).
#   - PHASE 3 (FIT)     : fits courts via erpm() (sanity: coefs finis).
#   - PHASE 4 (MCMC)    : probe multi-toggle (le point clé pour valider D_CHANGESTAT_FN).
#
# Important
#   - La PHASE 4 vise uniquement à déclencher le chemin multi-toggle côté C.
#     Pour voir les traces, recompiler avec DEBUG_COV_DIFF_GW=1 dans le C,
#     ou activer des traces conditionnelles déjà présentes.
# ==============================================================================
...
summary_on_bipartite_network <- function(part, nodes, rhs_txt) {
  ...
  # IMPORTANT:
  # - En multi-toggle, le changestat est D_ et la présence de d_func=TRUE
  #   dans l'InitErgmTerm est indispensable.
...
expected_cov_diff <- function(part, x, clique_size = 2L) {
...
expected_cov_diff_GW <- function(part, x, lambda) {
  ...
  ck <- vapply(2:Kmax, function(k) {
    expected_cov_diff(part, x, clique_size = k)
  }, numeric(1))
...
run_phase1_summary_expected_cov_diff_GW <- function() {
  ...
  for (l in lambdas) {
    ...
    sA   <- summary_on_bipartite_network(partA, nodesA, sprintf("cov_diff_GW('score', lambda = %g)", l))
    cat(sprintf("[A] lambda=%g  obtenu=%g  attendu=%g\n", l, sA, expA))
    stopifnot(isTRUE(all.equal(as.numeric(sA), as.numeric(expA))))
  }

  for (l in lambdas) {
    expB <- expected_cov_diff_GW(partB, nodesB$score, lambda = l)
    sB   <- summary_on_bipartite_network(partB, nodesB, sprintf("cov_diff_GW('score', lambda = %g)", l))
    cat(sprintf("[B] lambda=%g  obtenu=%g  attendu=%g\n", l, sB, expB))
    stopifnot(isTRUE(all.equal(as.numeric(sB), as.numeric(expB))))
  }

  for (l in lambdas) {
    expC <- expected_cov_diff_GW(partC, nodesC$score, lambda = l)
    sC   <- summary_on_bipartite_network(partC, nodesC, sprintf("cov_diff_GW('score', lambda = %g)", l))
    cat(sprintf("[C] lambda=%g  obtenu=%g  attendu=%g\n", l, sC, expC))
    stopifnot(isTRUE(all.equal(as.numeric(sC), as.numeric(expC))))
  }

  expA_v <- expected_cov_diff_GW(partA, nodesA$score, lambda = c(2, 4))
  sA_v   <- summary_on_bipartite_network(partA, nodesA, "cov_diff_GW('score', lambda = c(2,4))")
  cat(sprintf("[A] lambda=c(2,4)  obtenu=%s  attendu=%s\n",
              paste(sA_v, collapse = ","), paste(expA_v, collapse = ",")))
...
# ======================================================================================
# Phase 3: Fits courts via erpm() — MLE + loglik (sanity)
# ======================================================================================
...
run_phase3_erpm_fits_cov_diff_GW <- function() {
  cat("\n=== PHASE 3 : Fits erpm() (sanity) pour cov_diff_GW ===\n")

  fits <- list(
    A_l2 = run_fit_cov_diff_GW(partA, nodesA, "cov_diff_GW('score', lambda = 2)", "A_l2"),
    A_l2_l4 = run_fit_cov_diff_GW(partA, nodesA, "cov_diff_GW('score', lambda = c(2,4))", "A_l2_l4"),
    B_l2 = run_fit_cov_diff_GW(partB, nodesB, "cov_diff_GW('score', lambda = 2)", "B_l2")
  )
...
.run_mcmc_multitoggle_probe <- function(nw, rhs_txt) {
  ...
  #   - observer des traces côté C quand ntoggles > 1
  #
  # NOTE:
  #   - Le fait d'obtenir *effectivement* des propositions multi-toggle dépend
  #     du proposal. `~ sparse` est un bon candidat générique pour voir passer
  #     des ntoggles > 1 (selon version/contrainte).
...
run_phase4_mcmc_probe <- function() {
  ...
  cat("Objectif: voir passer des traces côté C indiquant ntoggles > 1.\n")
  cat("Si tu ne vois rien:\n")
  cat("  - recompiler avec DEBUG_COV_DIFF_GW=1 dans src/changestat_cov_diff_GW.c\n")
  cat("  - augmenter burnin / changer MCMC.prop.\n\n")

  # Réseau cible: cas A (déterministe)
  nw <- .erpm_build_bipartite_nw(partA, nodesA)

  # On active le debug R de l'initializer (optionnel)
  options(ERPM.cov_diff_GW.debug = TRUE)

  .run_mcmc_multitoggle_probe(nw, "cov_diff_GW('score', lambda = c(2,4))")

  options(ERPM.cov_diff_GW.debug = FALSE)

  cat("\nSi un message du type '[cov_diff_GW] MULTI-TOGGLE ntoggles=...' apparaît, probe OK.\n")
  invisible(TRUE)
}
...
set.seed(1)
cat("=== TEST ERPM: cov_diff_GW (multi-toggle) ===\n")
