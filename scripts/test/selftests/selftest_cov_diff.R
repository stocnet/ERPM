# ==============================================================================
# Fichier : scripts/test/selftests/selftest_cov_diff.R
# Objet   : Self-test autonome pour l'effet ERPM/ERGM `cov_diff`
...
# Pour chaque groupe g de taille n_g >= k :
#   - toutes les k-cliques S subset g
#   - D(S) = max(x_i : i in S) - min(x_i : i in S)
# Stat non normalisée :
#   T_k = sum_g sum_{S in C_k(g)} D(S)
# Stat normalisée par groupe :
#   T_k^{by_group} = sum_g [ (1/choose(n_g,k)) * sum_{S in C_k(g)} D(S) ]
expected_cov_diff <- function(part, x, clique_size = 2L, normalized = FALSE) {
    ...
    for (ix in split_idx) {
        ...
        # idx_mat: k x nb_cliques, indices relatifs 1..ng
...
run_phase1_summary_expected_cov_diff <- function() {
    cat("=== PHASE 1 : Summary cov_diff avec attentes explicites ===\n")

    # ---------- Cas A (n=15, tailles 4,5,6) ----------
    ...
    cat(sprintf("[A] cov_diff('score',k=2)              obtenu=%g  attendu=%g\n", s_A_k2_raw, exp_A_k2_raw))
    ...
    cat(sprintf("[A] cov_diff('score',k=2,norm=TRUE)    obtenu=%g  attendu=%g\n", s_A_k2_norm, exp_A_k2_norm))
    ...
    cat(sprintf("[B] cov_diff('score',k=2)              obtenu=%g  attendu=%g\n", s_B_k2_raw, exp_B_k2_raw))
    ...
    cat(sprintf("[B] cov_diff('score',k=2,norm=TRUE)    obtenu=%g  attendu=%g\n", s_B_k2_norm, exp_B_k2_norm))
    ...
    cat(sprintf("[C] cov_diff('score',k=2)              obtenu=%g  attendu=%g\n", s_C_k2_raw, exp_C_k2_raw))
    ...
    cat(sprintf("[C] cov_diff('score',k=2,norm=TRUE)    obtenu=%g  attendu=%g\n", s_C_k2_norm, exp_C_k2_norm))
    ...
    cat(sprintf("[C] cov_diff('score',k=3)              obtenu=%g  attendu=%g\n", s_C_k3_raw, exp_C_k3_raw))
...
check_summary_equivalence_cov_diff <- function(part, nodes, rhs_vec) {
    for (rhs in rhs_vec) {
        ...
        cat(sprintf("[EQUIV-DIFF] n=%-3d RHS=%-45s net=%s | erpm=%s\n",
...
run_phase2_summary_equiv_cov_diff <- function() {
...
run_fit_cov_diff <- function(part, nodes, rhs, tag) {
    ...
    if (!inherits(s_obs, "try-error")) {
        ...
        cat(sprintf("[ERPM-FIT-COVDIFF %-12s] stat_observee=NA (%s)\n", tag, conditionMessage(attr(s_obs, "condition"))))
    }
...
run_phase3_erpm_fits_cov_diff <- function() {
    cat("\n=== PHASE 3 : Fits erpm() (MLE + loglik) pour cov_diff (avec squared_sizes) ===\n")
...
set.seed(1)
cat("=== TEST ERPM: cov_diff ===\n")
run_phase1_summary_expected_cov_diff()
run_phase2_summary_equiv_cov_diff()
res_fits_cov_diff <- run_phase3_erpm_fits_cov_diff()
